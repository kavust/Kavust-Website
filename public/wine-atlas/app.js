...
function select(id,focus=false){
 ...
 document.querySelectorAll('[data-country]').forEach(b=>{b.classList.toggle('selected',b.dataset.country===id);b.setAttribute('aria-pressed',String(b.dataset.country===id));});
...
// Resize the same-origin embedded atlas to its content, including mobile profiles.
if(window.parent!==window){
 let resizeFrame=0;
 const report=()=>{cancelAnimationFrame(resizeFrame);resizeFrame=requestAnimationFrame(()=>window.parent.postMessage({type:'kavust-atlas-height',height:Math.ceil(document.body.getBoundingClientRect().height)},location.origin));};
